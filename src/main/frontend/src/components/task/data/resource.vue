    ...
    <div class="d-flex flex-row" style="width: 100%; height: 0; flex: 1">
      <v-navigation-drawer v-model="drawerOpen" :width="drawerOpen ? 350 : 0" mobile-breakpoint="0">
        ...
        <v-list three-line style="padding: 0 0px; border-radius: 6px">
          <application-instance-data-item
            ...
            :selected="data && data.url === item.url"
            :key="item.url"
            :item="item"
          />
        </v-list>
      ...
      >
        ...
        >
          ...
          <div v-if="data.id || data.url" class="detail-name">
            {{ (data.configJson && data.configJson.name) || data.name }}
          </div>
          <div v-if="data.id" class="detail-key" style="margin-right: 12px">|</div>
          <div v-if="data.id" class="detail-key">
            {{ data.configJson && data.configJson.desc }}
          </div>
          <v-spacer />
          <v-divider vertical />
          <v-btn
            text
            :disabled="!data.url"
            @click="$refs.iframeContainer.src = `${data.url}`"
...
export default {
  ...
  methods: {
    async onMounted() {
      ...
      this.dataList.items.push(this.data);
      this.dataList.count = 1;
      this.idBreadcrumbs[1].text = this.data.name;
    },
