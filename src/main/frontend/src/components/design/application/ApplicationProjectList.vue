    ...
    <v-card class="detail-card" style="border-top: 1px solid rgba(187, 187, 187, 1);">
      <v-layout style="margin: 0" class="stretch">
        <div style="display: flex; flex-direction: column; width: 100%">
          <el-row type="flex" justify="start" class="menu-border" style="border-bottom-width: 0px">
            <span style="height:100%;font-size:14px; line-height:44px;margin-left:15px;color:#83829A;">{{ $t('onlineTenant') }}</span>
            <v-select
            ...
            >
              <v-icon size="16" color="#83829A" style="margin-right:6px;"> mdi-magnify</v-icon>
            ...
            <v-btn large text :disabled="!group" class="process-menu" @click="create()">
              ...
              <span style="color: #444262;font-size:14px;">{{ $t('创建组件项目') }}</span>
            </v-btn>
          </el-row>
              ...
              <el-table-column :label="$t('collaborator')">
                <template slot-scope="props">
                  <v-chip
                      ...
                      font-size: 12px;padding: 8px 8px;border-radius: 8px !important;"
                      :style="index>0?{marginLeft:'-20px !important'}:{marginLeft:'0 !important'}"
                      :key="member.id"
                  >
                    {{ member.name }}
                  </v-chip>
                  <v-chip
                      ...
                      text-color="white"
                      style="border: 2px solid rgba(255, 255, 255, 1);
                      font-size: 12px;padding: 8px 8px;border-radius: 8px !important;margin-left: -20px!important;"
                  >
                    ...
                    <v-list style="cursor: pointer" dense>
                      <v-list-item key="rename">
                        <v-list-item-title @click="rename()">
                          <v-icon size="16" color="#83829a" style="margin-right: 6px; margin-bottom:3px">
                            mdi-rename-box
                          </v-icon>
                      ...
                      >
                        <v-list-item-title>
                          <v-icon size="16" color="#e91e63" style="margin-right: 6px; margin-bottom:3px">
                            mdi-delete-forever
                          </v-icon>
          ...
          <v-form ref="form" lazy-validation>
            <v-row style="margin: 0">
              <v-col style="padding: 0" cols="12">
                <v-text-field
                    :label="$t('name')"
                    :placeholder="$t('plsInputProjectName')"
    ...
    <v-dialog v-model="searchDialog.display" max-width="300">
      <v-card>
        ...
        <v-card-actions style="padding-bottom: 16px">
          ...
          <v-btn style="color: #ffffff" color="#ff7754" box-shadow="none" @click="getProjects()">
            <v-icon size="16" color="#ffffff" style="margin-right: 6px">
              mdi-magnify
            </v-icon>
            <span>{{ $t('query') }}</span>
...
export default {
  ...
  watch: {
    ...
    "$i18n.locale": {
      handler(val) {
        let self = this;
        self.breadcrumbs[0].text = self.$t("app");
      }
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async getData() {
      ...
      if (this.groups.length) {
        ...
        if (this.$route.query.defaultGroup) {
          currentGroupIndex = this.groups.findIndex(group => group.id == this.$route.query.defaultGroup);
        }
    ...
    async createProject() {
      ...
      this.$message({
        message: this.$t("createProjectSuccessWithName",{name:this.createDialog.name}),
        type: "success",
        showClose:true
      });
      await this.getProjects();
    ...
    async updateProject() {
      ...
      this.$message({
        message: this.$t("updateProjectSuccessWithName",{name:this.updateDialog.name}),
        type: "success",
        showClose:true
      });
      await this.getProjects();
    ...
    async deleteProject() {
      ...
      this.$message({
        message: this.$t("deleteProjectSuccessWithName",{name:this.project.name}),
        type: "success",
        showClose:true
      });
      await this.getProjects();
