<template>
  <v-layout style="margin: 0; height: 100%" class="stretch">
    <div style="display: flex; flex-direction: column; width: 100%">
      ...
      >
        <v-icon size="16" color="#D500F9" style="margin-right: 10px">
          mdi-alpha-f-circle
        </v-icon>
    ...
    <v-dialog v-model="createResourceDialog.display" max-width="300">
      <v-card>
        ...
        <v-card-actions style="padding-bottom: 16px">
          <v-spacer/>
          <v-btn text @click="createResourceDialog.display = false">
            {{ $t('cancel') }}
          </v-btn
          >
          <v-btn
        ...
        <v-card-text>
          <div class="d-flex flex-column" style="max-height: calc(90vh - 150px);width: 100%;height: 100%">
            <v-text-field
                ...
                dense
                hide-details
                @change="startImportObject"
                v-model="objectDialog.token"
            />
            <div class="d-flex flex-row" style="width: 100%;gap: 15px;margin-top: 12px">
...
export default {
  ...
  watch: {
    "formBuilder.show": {
      handler(val) {
        // console.log("watch formBuilder.show", val);
      },
    },
    $route: {
      handler() {
  ...
  methods: {
    ...
    async createResource() {
      ...
      if (this.createResourceDialog.type === 4) {
        // 表单
        try {
      ...
      this.$emit("onCreated", {
        id: resourceResult.id, type: {value: 4}, name: self.createResourceDialog.name, projectId: self.project.id
      });
    },
    async startImportObject() {
      await this.selectProject({});
      let {list} = await nocodbRequest.getProjects(this.objectDialog.token);
    ...
    async selectProject(project) {
      ...
      try {
        let {list} = await nocodbRequest.getTables(
            project.id,
            this.objectDialog.token
        );
    ...
    async importObject() {
      this.formBuilder.show = false;
      let table = await nocodbRequest.getTable(
          this.objectDialog.table.id,
          this.objectDialog.token
      );
